...
| **Primary metric** | Per-library Pearson *r* of `(pred, log2mRNA)`, computed separately against each replicate column (`rep1`, `rep2`) on the two-replicate libraries (GFP r1 / GFP r2) and against the single column on TDH3. Spearman ρ reported alongside. We deliberately do **not** merge replicates into one label — the two per-replicate Pearsons against the published replicate-replicate ceiling are the headline. Cross-library aggregate Pearson reported per replicate as a single number after per-library z-score. |
...
4. **Per-library Pearson *r*** on `(pred, log2mRNA)`, computed **separately per replicate** for the two-replicate libraries: `pearson_rep1` and `pearson_rep2` for GFP r1 / GFP r2, and a single `pearson` for TDH3. Spearman ρ alongside. We deliberately do not pre-average replicates into one label — reporting both Pearsons against the published replicate-replicate ceiling is more informative than collapsing them.
