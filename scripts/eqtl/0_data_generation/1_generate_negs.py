        ...
        with open(gwas_file, "r") as f:
            ...
            for rc in required_cols:
                if rc not in col_map:
                    raise ValueError(
                        f"Missing required column '{rc}' in header: {header}"
                    )
...
def pick_random_gene_within_distance(
    variant_chrom, variant_pos, desired_distance, tolerance, tss_data, max_tries=50000
):
    """
    ...
    same_chr_genes = [
        gene_id
        for gene_id, (chr_g, _, _) in tss_data.items()
...
def main(
    ...
    positive_variant_keys = set(
        (eq["chrom"], eq["pos"], eq["ref"], eq["alt"]) for eq in positive_eqtls
    )
    positive_triplets = set(
        (eq["chrom"], eq["pos"], eq["gene"]) for eq in positive_eqtls
    )
    ...
    for iteration in range(1, iterations + 1):
        ...
        print(
            f"Iteration {iteration} complete. Found {len(negative_results)} negative eQTLs, with {len(summary_data)} summary rows."
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate distance-matched negative eQTLs."
    )
    parser.add_argument("--gwas_file", required=True, help="Input positive eQTLs file")
    parser.add_argument(
        "--gvcf_file", required=True, help="Input gVCF file for non-coding variants"
    )
    parser.add_argument(
        "--gtf_file", required=True, help="Input GTF file for TSS mapping"
    )
    parser.add_argument(
        "--output_prefix",
        required=True,
        help="Output file prefix (e.g. results/negative_eqtls)",
    )
    parser.add_argument(
        "--dataset",
        choices=["caudal", "kita", "renganaath"],
        default="caudal",
        help="Dataset format parsing",
    )
    parser.add_argument(
        "--distance_tolerance",
        type=int,
        default=100,
        help="Tolerance for matching TSS distance",
    )
    parser.add_argument(
        "--negatives_per_positive",
        type=int,
        default=1,
        help="Number of negative variants per positive variant",
    )
    parser.add_argument(
        "--iterations", type=int, default=4, help="Number of negative sets to generate"
    )
