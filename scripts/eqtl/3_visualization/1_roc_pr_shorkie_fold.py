...
def process_shorkie(exp: str, neg_set: int) -> pd.DataFrame:
    ...
    base_dir = os.path.join(
        ROOT, "revision_experiments", "eQTL", "viz_new", "results", f"negset_{neg_set}"
    )
    ...
    df_orig = pd.read_csv(
        orig_path, sep="\t", usecols=["Position_Gene", "logSED_agg", "label"]
    )
...
def get_mpra_base(exp):
    if exp == "caudal_etal":
        return os.path.join(
            ...
            "SUM_data_process",
            "eQTL",
            "eqtl_MPRA_modeals_eval",
            "eQTL_MPRA_models_eval_caudal_etal",
            "results",
        )
    elif exp == "kita_etal":
        return os.path.join(
            ...
            "eQTL",
            "eqtl_MPRA_modeals_eval",
            "eQTL_MPRA_models_eval_kita_etal_select",
            "results",
        )
        ...
        return os.path.join(
            ...
            "eQTL",
            "eQTL_MPRA_models_eval_Renganaath_etal",
            "results",
        )
    return None
